...
def from_text(src, dst):
	fp = open(src, "r")
	w = int(fp.readline().strip(), 16)
	h = int(fp.readline().strip(), 16)
	buff = ctypes.create_string_buffer(4 * w * h)
	offset = 0
	for _ in xrange(h):
		for _ in xrange(w):
			val = bin(int(fp.readline().strip(), 16))[2:].zfill(32)
			r = chr(int(val[:8], 2))
			g = chr(int(val[8:16], 2))
			b = chr(int(val[16:24], 2))
			struct.pack_into('cccc', buff, offset, r, g, b, '\0')
			offset += 4;
	out = open(dst, 'wb')
	out.write(struct.pack('ii', w, h))
	...
	if show:
		...
		if f1 and not f2 and not f3:
			...
			if not alfa:
				...
				print 'Error alfa'
		elif f3 and not f1 and f2:
			from_text(argv[0], argv[1])
